...
```json
{
  "targetResource": "//chat.googleapis.com/SPACE_RESOURCE_PLACEHOLDER",
  "eventTypes": [
...
- `data` field (base64-decoded, JSON) with `includeResource=true`:
  ...
  {
    "message": {
      "name": "SPACE_MESSAGE_RESOURCE_PLACEHOLDER",
      "sender": {"name": "USER_RESOURCE_PLACEHOLDER", "type": "HUMAN"},
      "text": "hello",
...
## Concrete next actions for DEMO 1 on Example Chat Assistant

1. Verify `chat-api-push@system.gserviceaccount.com` has `roles/pubsub.publisher` on `projects/example-gchatbot-project/topics/chat-events`. (Briefing says yes; re-verify via Cloud Console → Pub/Sub → Topic → Permissions.)
2. Workspace admin: approve scopes `chat.app.messages.readonly`, `chat.app.spaces.readonly`, `chat.app.memberships` for the Example Chat Assistant Chat app.
3. From SA credentials with those scopes: `POST /v1/subscriptions` with the body shape above, targeting `SPACE_RESOURCE_PLACEHOLDER` + `DM_SPACE_RESOURCE_PLACEHOLDER` (the bot's DM with Example User) — two subscriptions, or one per target resource.
4. Verify subscription shows `state: "ACTIVE"` via `subscriptions.list()` (SA needs read scope too).
5. Example user sends `hello` → Pub/Sub topic receives a message → gateway pulls it → adapter logs it.
