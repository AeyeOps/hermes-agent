...
3. **Placeholder UX tuning** — is `💭 _Thinking…_` the right default, or should it say `assistant` explicitly? Try both in example-test-space after ship.
...
5. **Card width** — cardsV2 default width may feel narrower than inline text. Consider `headerImageAltText`/header to give it visual anchor, or accept default. Smoke-verify in example-test-space.
...
**DEMO #4** (after M6 lands): in `example-test-space`, send a prompt likely to produce a nested list or code block. Verify:
