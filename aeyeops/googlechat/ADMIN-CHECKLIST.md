...
Hand this file to the `moodmedia.com` Workspace admin, then flip the
...
**Where:** <https://console.cloud.google.com/apis/api/appsmarket-component.googleapis.com/googleapps_sdk?project=sa-mm-gchatbot>
...
Find the **Mood Media Assistant** listing (project
`sa-mm-gchatbot`, service-account client ID `110945615708798543371`).
...
**Where:** Google Chat client — open the space named **steve-test**.

The space already exists (`spaces/AAQA2N6jyoA` — verified
2026-04-24). Confirm that **Mood Media Assistant** is in the member
list. If it was removed during earlier testing, re-add via `@Mood Media
Assistant`.
...
export GOOGLECHAT_PUBSUB_TOPIC=projects/sa-mm-gchatbot/topics/chat-events
```
...
`~/.mood-media-assistant/key.json` and don't create lasting tenant
state. They run in sequence against the existing `steve-test` space.
