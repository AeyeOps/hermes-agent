...
> aeyeops/googlechat/build-plan.md`** — the system plan-mode gate
...
and identifies the shortest path to a live bot reply in `SPACE_RESOURCE_PLACEHOLDER`
(`example-test-space`) via the provisioned `chat-events-sub` pull subscription.
...
```
project_id      example-gchatbot-project                  (parent: example.com org)
topic           projects/example-gchatbot-project/topics/chat-events
subscription    projects/example-gchatbot-project/subscriptions/chat-events-sub
service_account example-chat-assistant@example-gchatbot-project.iam.gserviceaccount.com
key_path        ~/.example-chat-assistant/key.json   (chmod 600, parent dir 0700)
chat_app        "Example Chat Assistant"            (installed in SPACE_RESOURCE_PLACEHOLDER = example-test-space)
auth_tools      gcloud (~/google-cloud-sdk/bin/gcloud) as user@example.com w/ ADC
                workspace-example MCP (read-only Workspace/Chat against example tenant)
```

`chat-api-push@system.gserviceaccount.com` already has `roles/pubsub.publisher` on the
topic; `example-chat-assistant` SA has `roles/pubsub.subscriber` on the subscription. IAM
...
Goal: bot replies in `example-test-space` from a live message arriving via
...
export GOOGLECHAT_SERVICE_ACCOUNT_JSON=$HOME/.example-chat-assistant/key.json
export GOOGLECHAT_PUBSUB_PROJECT=example-gchatbot-project
export GOOGLECHAT_PUBSUB_SUBSCRIPTION=chat-events-sub
export GOOGLECHAT_ALLOWED_USERS=users/<example_user_id>   # captured during fixture pass

# 2. Start hermes-agent on this branch
hermes-agent gateway --platforms googlechat

# 3. In Google Chat, send a message in the example-test-space space (SPACE_RESOURCE_PLACEHOLDER)
...
Schedule a cronjob via the agent in `example-test-space` with delivery to a known
...
In `example-test-space`, ask the agent: "Show me a poll with two options for staging vs.
...
**🟢 DEMO #4** — after M6 lands: in `example-test-space`, prompt the bot for output
...
`example-test-space` once the adapter runs**. No coworker DM payloads.
...
1. After C9 lands, run the gateway with `LOG_LEVEL=DEBUG` (or wrap the
   ...
   line) and exchange messages with the bot in `SPACE_RESOURCE_PLACEHOLDER`.
2. Copy the logged event into a Python literal at the top of the relevant test
   file:
   ```python
   _MESSAGE_FIXTURE = {
       "type": "MESSAGE",
       "message": {"name": "SPACE_MESSAGE_RESOURCE_PLACEHOLDER",
                   "sender": {"name": "USER_RESOURCE_PLACEHOLDER", "displayName": "Example User",
                              "type": "HUMAN"},
                   "text": "hello",
                   "createTime": "2026-04-20T..."},
       "space": {"name": "SPACE_RESOURCE_PLACEHOLDER", "type": "DM"},
       "user": {"name": "USER_RESOURCE_PLACEHOLDER"},
   }
   ```
3. **Redact before committing**: replace any real user resource name with
   `USER_RESOURCE_PLACEHOLDER` and the real space ID with
   `SPACE_RESOURCE_PLACEHOLDER`. Keep the doubled-`messages/<id>.<id>` shape — that
...
|-------------|---------------------------|----------------|
| C5 (inbound MESSAGE) | MESSAGE event in DM (example-test-space) | Immediately after DEMO 1 — round-trip a "hello" message and copy the inbound payload from gateway logs |
...
| C23 (CARD_CLICKED) | CARD_CLICKED with action.parameters + formInputs | After C20 lands a `send_chat_card` impl that the implementer can manually invoke; click the card in example-test-space and capture the click event |
| C26 (media) | MESSAGE with `attachment` field | Send a small image to the bot in example-test-space; capture |
...
|------|--------------|----------------|
| **DEMO 1** — first reply | C9 | Message in `example-test-space` → bot replies via Chat REST. Inbound dedup works (no double-reply). Self-filter works (bot's own message doesn't loop). |
...
3. **Type check** — `pyright gateway/platforms/googlechat.py tools/send_chat_card_tool.py`
   clean.
4. **Live smoke (DEMO 1)** — bot replies in `example-test-space` from a live message.
