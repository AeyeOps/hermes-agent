...
Supports either a dedicated dashboard hostname root or a path prefix such as
https://mission-control.example.com/hermes/.
...
if [[ ${#PASSWORD_HASH} -gt 256 || ! "$PASSWORD_HASH" =~ ^\$[A-Za-z0-9\$./=,+_-]{10,255}$ ]]; then
...
log "domain=$DOMAIN hermes_home=$HERMES_HOME repo_dir=$REPO_DIR port=$PORT path_prefix=${PATH_PREFIX:-<root>} dry_run=$DRY_RUN enable_tui=$ENABLE_TUI"
...
if [[ -x "$REPO_DIR/.venv/bin/python" ]]; then
  ...
  echo "Hermes venv python not found/executable under: $REPO_DIR/.venv or $REPO_DIR/venv" >&2
  exit 1
fi
...
if [[ "$DRY_RUN" -eq 0 ]]; then
  ...
  # `caddy validate` is run as root and may eagerly create the access-log file.
  # Re-assert ownership before reloading the live caddy service, which runs as
  # the caddy user.
  ensure_caddy_log_dir
