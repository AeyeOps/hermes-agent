...
log "domain=$DOMAIN hermes_home=$HERMES_HOME port=$PORT log_file=$LOG_FILE"
...
if [[ "$SKIP_EXTERNAL" -eq 0 ]]; then
  ...
  unauth_tmp="$(mktemp)"
  unauth_code="$(curl "${tls_args[@]}" -sS -o "$unauth_tmp" -w '%{http_code}' "https://$DOMAIN/" || true)"
  rm -f "$unauth_tmp"
  ...
  if [[ -n "$AUTH_USER" && -n "$AUTH_PASSWORD" ]]; then
    auth_tmp="$(mktemp)"
    auth_code="$(curl "${tls_args[@]}" -sS -u "$AUTH_USER:$AUTH_PASSWORD" -o "$auth_tmp" -w '%{http_code}' "https://$DOMAIN/api/status" || true)"
    rm -f "$auth_tmp"
