...
_PLATFORMS = [
    ...
    },
    {
        ...
        "label": "Google Chat",
        "emoji": "💬",
        "token_var": "GOOGLECHAT_SERVICE_ACCOUNT_JSON",
        "setup_instructions": [
            "1. Create or choose a Google Cloud project for the Chat app",
            "2. Create a service account JSON key and grant Pub/Sub subscriber access to the app subscription",
            "3. Configure the Google Chat app to publish events to Cloud Pub/Sub",
            "4. Copy the service account JSON path, Pub/Sub project, and subscription ID",
            "5. Set a home space such as spaces/AAAA... for cron and notifications",
            "6. Optional chat.app.* capabilities require separate Workspace admin approval; this setup only records local config",
        ],
        "vars": [
            {"name": "GOOGLECHAT_SERVICE_ACCOUNT_JSON", "prompt": "Service account JSON path", "password": False,
             "help": "Absolute path to the Google Cloud service account JSON key."},
            {"name": "GOOGLECHAT_PUBSUB_PROJECT", "prompt": "Pub/Sub project ID", "password": False,
             "help": "Google Cloud project containing the Pub/Sub subscription."},
            {"name": "GOOGLECHAT_PUBSUB_SUBSCRIPTION", "prompt": "Pub/Sub subscription ID", "password": False,
             "help": "Subscription ID without the projects/.../subscriptions/ prefix."},
            {"name": "GOOGLECHAT_ALLOWED_USERS", "prompt": "Allowed users (comma-separated users/... IDs or emails)", "password": False,
             "is_allowlist": True,
             "help": "Users allowed to interact with this Google Chat bot."},
            {"name": "GOOGLECHAT_HOME_CHANNEL", "prompt": "Home space (spaces/...) for cron/notifications", "password": False,
             "help": "Google Chat space resource name used for default deliveries."},
            {"name": "GOOGLECHAT_PUBSUB_TOPIC", "prompt": "Workspace Events Pub/Sub topic (optional)", "password": False,
             "help": "Only needed for admin-approved Workspace Events subscriptions."},
        ],
    },
    {
        "key": "matrix",
